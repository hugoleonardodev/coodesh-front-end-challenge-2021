...
const server = setupServer(
    userResponse,
    rest.get('http://localhost:5010/true&nat=nation&page=2', async (_request, response, context) => {
        return response(context.json(firstTenPatients))
    }),
)
...
describe('getSearchQuerySumit response', () => {
    it('should return a list of patients', async () => {
        const patientsList = await getSearchQuerySumit([{ query: 'brazil', filter: 'nation' }], 2)
